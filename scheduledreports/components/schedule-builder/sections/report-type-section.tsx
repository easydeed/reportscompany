...
const mainReportTypes: { value: ReportType; icon: string; label: string; description: string }[] = [
  { value: "new_listings", icon: "📸", label: "New Listings", description: "Photo gallery of new homes" },
  { value: "market_update", icon: "📊", label: "Market Update", description: "Stats & trends for the area" },
  { value: "closed_sales", icon: "🏠", label: "Closed Sales", description: "Recently sold properties" },
]
...
export function ReportTypeSection({
  ...
  const summary = `${reportTypeLabels[reportType]} · Last ${lookbackDays} days`
            ...
            {mainReportTypes.map((type) => (
              <button
                ...
                className={cn(
                  ...
                  reportType === type.value ? "border-violet-500 bg-violet-50" : "border-border",
                )}
              >
