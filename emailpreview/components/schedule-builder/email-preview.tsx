...
export function EmailPreview({ state, branding, profile, area }: EmailPreviewProps) {
  const getHeaderTitle = () => {
    if (state.reportType === "new_listings_gallery" && state.audienceFilter && state.audienceFilter !== "all") {
      return `${state.audienceFilterName} – ${area || "Your Area"}`
    }
  ...
  return (
    <div className="mx-auto max-w-full font-sans text-sm">
      ...
      >
        {/* Logo Placeholder */}
        <div className="mb-4 mx-auto w-24 h-8 bg-white/20 rounded flex items-center justify-center text-xs font-semibold">
          TrendyReports
        </div>
      ...
      <div className="bg-white p-6">
        ...
        <div className="bg-stone-50 border border-stone-200 rounded-md p-3 mb-5 text-xs text-stone-600">
          The {area || "selected area"} market showed strong activity this month with median prices holding steady.
        ...
        <div className="text-center my-5">
          <button
            className="inline-block px-6 py-3 rounded-md text-white font-semibold text-sm"
        ...
        <div className="flex items-center gap-3 pt-5 mt-5 border-t border-stone-200">
          <div
            className="w-12 h-12 rounded-full bg-stone-200 flex items-center justify-center text-lg"
          ...
          <div>
            ...
            <div className="text-[11px] text-stone-500">{profile.jobTitle}</div>
            <div className="text-[11px]" style={{ color: branding.primaryColor }}>
      ...
      <div className="bg-stone-50 rounded-b-xl px-4 py-4 text-center text-[10px] text-stone-500">
        Powered by TrendyReports • <span className="underline cursor-pointer">Unsubscribe</span>
      </div>
    </div>
  )
}

function ReportContent({ state }: { state: ScheduleBuilderState }) {
  switch (state.reportType) {
    case "market_snapshot":
      return <MarketSnapshotContent />
    case "new_listings_gallery":
      return <NewListingsContent />
    case "closed":
      return <ClosedSalesContent />
    case "inventory":
      return <InventoryContent />
    case "price_bands":
      return <PriceAnalysisContent />
    case "open_houses":
...
function NewListingsContent() {
  return (
    <>
      {/* Hero Metric */}
      <div className="text-center mb-4">
        <div className="text-[10px] uppercase tracking-wide text-stone-500 mb-1">New Listings</div>
      ...
      {/* Listing Gallery */}
      <div className="mb-4">
        <div className="text-[10px] uppercase tracking-wide text-stone-500 mb-2">12 New Listings</div>
    ...
    <>
      ...
      <div>
        ...
        <div className="space-y-2">
          {[
            { label: "Entry Level", range: "$0-$500K", count: 12, color: "#22c55e" },
            { label: "Move-Up", range: "$500K-$1M", count: 28, color: "#3b82f6" },
            { label: "Luxury", range: "$1M+", count: 10, color: "#8b5cf6" },
          ].map((tier, i) => (
