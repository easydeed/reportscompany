  ...
  return (
    <div className="flex flex-col items-center gap-6">
      <div className="relative w-full flex items-center justify-center">
        ...
        <div className={`${aspectClass} mx-auto w-full overflow-hidden rounded-2xl border border-border bg-card shadow-xl`}>
          <div className={`${cards[current].gradient} px-6 py-5`}>
            <div className="flex items-center gap-3">
              ...
              <p className="text-base font-semibold text-white">{cards[current].title}</p>
            </div>
          </div>
      ...
      <div className="flex items-center gap-2">
        {cards.map((card, i) => (
          <button
            ...
            className={`h-2 rounded-full transition-all ${i === current ? "w-6 bg-[#6366F1]" : "w-2 bg-foreground/20"}`}
