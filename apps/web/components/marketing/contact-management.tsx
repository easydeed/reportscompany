...
import { DEMO_MARKET, relativeDaysAgo } from "@/lib/demo-data";
import {
  Search,
  Plus,
  Upload,
...
const contacts = [
  {
    ...
    phone: "(949) 555-0142",
    group: `${DEMO_MARKET.city} Buyers`,
    type: "Client",
    lastSent: relativeDaysAgo(3),
    opens: 12,
    color: "bg-[#6366F1]",
  },
  {
    initials: "MC",
    name: "Michael Chen",
    email: "m.chen@brokerage.com",
    phone: "(949) 555-0298",
    group: "Agent Partners",
    type: "Agent",
    lastSent: relativeDaysAgo(6),
    opens: 8,
    color: "bg-[#4338CA]",
  },
  {
    ...
    phone: "(949) 555-0377",
    group: "Newport Sellers",
    type: "Client",
    lastSent: relativeDaysAgo(9),
    opens: 15,
    color: "bg-[#818CF8]",
  },
  {
    ...
    email: "david.r@realty.com",
    phone: "(949) 555-0451",
    group: "Agent Partners",
    type: "Agent",
    lastSent: relativeDaysAgo(12),
    opens: 6,
    color: "bg-[#312E81]",
  },
  {
  ...
  },
  {
    initials: "RK",
    name: "Rachel Kim",
    email: "rachel.k@lending.com",
    phone: "(949) 555-0705",
    group: "Lending Partners",
    type: "Agent",
    lastSent: relativeDaysAgo(34),
    opens: 4,
    color: "bg-[#334155]",
  },
];
...
const groups = [
  ...
  { name: `${DEMO_MARKET.city} Buyers`, count: 84 },
  { name: "Newport Sellers", count: 56 },
  { name: "Agent Partners", count: 38 },
  { name: "OC Investors", count: 41 },
  { name: "Lending Partners", count: 28 },
];
      ...
      >
        <motion.div variants={fadeUp} className="text-center">
          <h2 className="text-4xl font-bold tracking-tight text-foreground md:text-5xl">
            Manage your sphere in one place
          </h2>
          <p className="mx-auto mt-4 max-w-2xl text-muted-foreground">
            Import your contacts, organize them into groups, and schedule the
            right reports to the right people. Track who opens and who clicks.
          </p>
        </motion.div>
              ...
              <div className="mt-3 space-y-1">
                {groups.map((g, i) => (
                  ...
                  >
                    <span>{g.name}</span>
                    <span
                      className={`text-xs ${i === 0 ? "text-[#6366F1]" : "text-muted-foreground/60"}`}
                    >
                      {g.count}
                    </span>
                  </div>
            ...
            <div className="flex-1">
              ...
              {contacts.map((c) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <div
                      className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-xs font-bold text-white ${c.color}`}
                    >
                      {c.initials}
                    </div>
                    <div>
                  ...
                  <span
                    className={`hidden w-20 rounded-full px-2.5 py-0.5 text-center text-xs font-medium md:block ${
                      c.type === "Client"
              ...
              <div className="flex items-center justify-between border-t border-border bg-[#FAFBFC] px-6 py-3">
                <p className="text-xs text-muted-foreground">
                  Showing 7 of 247 contacts
                </p>
                <div className="flex items-center gap-1">
                  <div className="rounded-md border border-border px-2.5 py-1 text-xs font-medium text-muted-foreground">
                    Previous
                  </div>
                  <div className="rounded-md bg-[#6366F1] px-2.5 py-1 text-xs font-medium text-white">
                    1
                  </div>
                  <div className="rounded-md border border-border px-2.5 py-1 text-xs font-medium text-muted-foreground">
                    2
        ...
        <motion.div variants={fadeUp} className="mt-10">
          ...
          <div className="mt-4 flex flex-wrap items-center justify-center gap-4">
            {[
              { icon: Users, label: "Groups for easy scheduling" },
              { icon: Upload, label: "CSV import to get started fast" },
              { icon: Mail, label: "One-click delivery to any group" },
              { icon: BarChart3, label: "See who opens and who clicks" },
            ].map((p) => (
              <div
                key={p.label}
