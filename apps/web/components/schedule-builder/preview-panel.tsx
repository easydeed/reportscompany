...
import type { ScheduleBuilderState, ReportType } from "./types"

interface PreviewPanelProps {
  state: ScheduleBuilderState
}

const reportTypeLabels: Record<ReportType, { icon: string; label: string }> = {
  new_listings: { icon: "📸", label: "New Listings" },
  new_listings_gallery: { icon: "📸", label: "New Listings" },
  market_snapshot: { icon: "📊", label: "Market Update" },
  closed: { icon: "🏠", label: "Closed Sales" },
  inventory: { icon: "📦", label: "Inventory" },
  price_bands: { icon: "💰", label: "Price Bands" },
  open_houses: { icon: "🚪", label: "Open Houses" },
  featured_listings: { icon: "⭐", label: "Featured Listings" },
}

const dayNames = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"]

export function PreviewPanel({ state }: PreviewPanelProps) {
  const reportInfo = reportTypeLabels[state.reportType] || { icon: "📄", label: state.reportType }
  ...
  const timezoneShort = state.timezone === "America/Los_Angeles" ? "PT" 
    : state.timezone === "America/New_York" ? "ET"
    : state.timezone === "America/Chicago" ? "CT"
    : "MT"
    ...
    <div className="sticky top-24">
      <div className="rounded-xl border bg-background shadow-sm">
        ...
        <div className="p-4">
          <div className="rounded-lg border bg-muted/30">
            ...
            <div className="px-4 pb-4">
              <Button
                variant="outline"
                className="w-full text-violet-600 border-violet-200 hover:bg-violet-50 bg-transparent dark:text-violet-400 dark:border-violet-800 dark:hover:bg-violet-950"
              >
                View Full Report →
              </Button>
            </div>
        ...
        <div className="border-t px-4 py-3">
          <div className="flex items-center gap-2 rounded-lg bg-violet-50 px-3 py-2 text-sm dark:bg-violet-950/50">
...
function ReportPreviewContent({ reportType }: { reportType: ReportType }) {
  switch (reportType) {
    ...
    case "new_listings_gallery":
      return (
        <div className="space-y-2">
          ...
          <div className="pt-2 text-center text-sm text-violet-600 dark:text-violet-400">View All Listings →</div>
        </div>
      )
    case "closed":
      return (
      ...
      return (
        <div className="space-y-3">
          <div className="text-center">
            <div className="text-3xl font-bold text-violet-600 dark:text-violet-400">127</div>
    ...
    default:
      return (
        <div className="text-center text-sm text-muted-foreground py-4">
          Preview will update based on report type
        </div>
      )
  }
}
...
function OpenHouseCard({
  ...
  return (
    <div className="flex items-center justify-between rounded-lg border bg-background p-2">
      ...
      <div className="text-sm font-medium text-violet-600 dark:text-violet-400">{price}</div>
    </div>
  )
}
