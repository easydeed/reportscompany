...
class AuthContextMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if path.startswith("/health") or path.startswith("/docs") or path.startswith("/redoc") or path.startswith("/openapi") or path.startswith("/v1/auth/") or path.startswith("/dev-files/"):
            return await call_next(request)
